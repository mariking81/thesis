...
plot_NFD_DCM_Heatmap<-function(colorh1_NFD_DCM,AdjMatNFD,AdjMatDCM, expr_nfd_cpmlog, expr_dcm_cpmlog,ordering=NULL,file="DifferentialPlot.png")
{
  if (is.null(ordering))
  {
    h<-hclust(as.dist(1-abs(cor(getEigenGeneValues(expr_nfd_cpmlog[,which(colorh1_NFD_DCM!="grey")],colorh1_NFD_DCM[which(colorh1_NFD_DCM!="grey")],
                                                   rbind(expr_nfd_cpmlog,expr_african_american_cpmlog)[,which(colorh1_NFD_DCM!="grey")])))))
...
expr_caucasian <- read.csv("~/MSc/project/DiffCoEx/C_DCM_matrix.csv",row.names = 1)
expr_african_american <- read.csv("~/MSc/project/DiffCoEx/AA_DCM_matrix.csv", row.names = 1)
...
#merges clusters which are close together
...
write.table(colorh1_C_AA, file = "AA_C_DCM_module_assignment.txt", row.names = F, col.names = F, quote = F)
